...
require_once 'connect.php';
    ...
    if(isset($_POST['email']))
    {
        ...
        $func = $_POST['func'];
        if(preg_match($check, $func) == FALSE)
         {
            $allIsOk = FALSE;
            $_SESSION['e_func'] = "Funkcja może się składać tylko z liter i cyfr (Z polskimi znakami), musi zawierać minimum 2 znaki, oraz pierwsza litera musi być duża.";
         }
        if(strlen($func)==0)
        {
            $allIsOk = FALSE;
            $_SESSION['e_func'] = "Funkcja nie może pozostać pusta.";
        }
        ...
        try
        {
             ...
             if ($connection->connect_errno!=0) 
                 ...
                 else
                 {
                    ...
                    $func = mysqli_real_escape_string($connection, $func);
                ...
                if( $allIsOk == TRUE)
                {
                    $funcResult = $connection->query("SELECT `Id_obslugi` FROM `obsluga` WHERE  Funkcja='$func'");
                    if(!$funcResult)  throw new Exception( $connection->error);
                     
                     $numberOffunc = $funcResult->num_rows;
                     
                     if($numberOffunc>0)
                     {
                         $funcRow = mysqli_fetch_assoc($funcResult);
                     }else
                     {
                        $connection->query("INSERT INTO `obsluga`(`Funkcja`) VALUES ('$func')");
                        $funcResult = $connection->query("SELECT `Id_obslugi` FROM `obsluga` WHERE  Funkcja='$func'");
                        $funcRow = mysqli_fetch_assoc($funcResult);
                    ...
                    if($connection->query("INSERT INTO `uzytkownicy`(`Id_uzytkownika`, `Imie`, `Nazwisko`, `E_mail`, `Login`, `Haslo`,    `Password_hash`,`Salt`, `Numer_telefonu`, `Data_urodzenia`, `Numer_domu`, `Id_ulicy`,                 `Id_pacjenta`, `Id_lekarza`, `Id_obslugi`)"
                                                         . " VALUES (     NULL,       '$name', '$surname', '$email', '$nick','$password1','$passwordHash','$salt','$phoneNumber',    '$dateOfBirth',  '$houseNumber' ,'$streetRow[Id_ulicy]',        NULL,        NULL,  '$funcRow[Id_obslugi]')"))
                    {
...
<html>
    <head>
        ...
        <style>
            ...
            body
            {
                background-image:url('images/cherbata.jpg');
            ...
            <div id="login">
                ...
                <form method="post">
                    ...
                    <?php
                        ...
                        {
                            echo '<div class="error">'.$_SESSION['e_postalCode'].'</div>';
                    ...
                    Funkcja:</br> <input type="text" value="<?php
                    if(isset($_SESSION['fs_func']))
                    {
                        echo $_SESSION['fs_func'];
                        unset($_SESSION['fs_func']);
                    }
                    ?>"  name="func" /> </br>
                    <?php
                        if(isset($_SESSION['e_func']))
                        {
                            echo '<div class="error">'.$_SESSION['e_func'].'</div>';
                            unset($_SESSION['e_func']);
                        }
                    ?>
