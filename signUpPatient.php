...
require_once 'connect.php';
    ...
    if(isset($_POST['email']))
    {
        ...
        $checkDateOfBirth = "(?:19|20)[0-9]{2}-(?:(?:0[1-9]|1[0-2])-(?:0[1-9]|1[0-9]|2[0-9])|(?:(?!02)(?:0[1-9]|1[0-2])-(?:30))|(?:(?:0[13578]|1[02])-31))";

        $name=$_POST['name'];
         if(preg_match($checkName, $name) == FALSE)
        {
            $allIsOk = FALSE;
            $_SESSION['e_name'] = "Imie może się składać tylko z liter i cyfr, oraz pierwsza litera musi być duża.";
        }
        if(strlen($name)==0)
        {
            $allIsOk = FALSE;
        ...
        {
            $allIsOk = FALSE;
            $_SESSION['e_street'] = "Ulica może się składać tylko z liter i cyfr, musi zawierać minimum 2 znaki, oraz pierwsza litera musi być duża.";
        }
        if(strlen($street)==0)
        {
            $allIsOk = FALSE;
        ...
        try
        {
             ...
             if ($connection->connect_errno!=0) 
                 ...
                 else
                 {
                    ...
                    $pesel = mysqli_real_escape_string($connection, $pesel);
                    ...
                    $passwordHash = sha1($stringToHash);
                     ...
                     $numberOfPesel = $result->num_rows;
                     if($numberOfPesel>0)
                     {
                         $allIsOk = false;
                         $_SESSION['e_pesel']="Istnieje już konto przypisane do tego pesla.";
                     }
                ...
                if( $allIsOk == TRUE)
                {
                    $connection->query("INSERT INTO `pacjenci`(`Pesel`) VALUES ('$pesel')");
                    $pacjentResult = $connection->query("SELECT `Id_pacjenta`FROM `pacjenci` WHERE Pesel='$pesel'");
                    $pacjentRow = mysqli_fetch_assoc($pacjentResult);
                    ...
                    if($connection->query("INSERT INTO `uzytkownicy`(`Id_uzytkownika`, `Imie`, `Nazwisko`, `E_mail`, `Login`, `Haslo`,    `Password_hash`,`Salt`, `Numer_telefonu`, `Data_urodzenia`, `Numer_domu`, `Id_ulicy`, `Id_pacjenta`, `Id_lekarza`, `Id_obslugi`)"
                                                         . " VALUES (     NULL,       '$name', '$surname', '$email', '$nick','$password1','$passwordHash','$salt','$phoneNumber',    '$dateOfBirth',  '$houseNumber' ,'$streetRow[Id_ulicy]',        '$pacjentRow[Id_pacjenta]',          NULL,         NULL)"))
                    {
        ...
        <div id="container">
            ...
            <div id="login">
                ...
                <form method="post">
                    ...
                    {
                        $stringEmail = preg_replace('/\s+/', '', $_SESSION['fs_email']);
                        echo $stringEmail;
                    ...
                    <?php
                        ...
                        {
                            echo '<div class="error"><span>'.$_SESSION['e_pesel'].'</span></div>';
