...
require_once 'connect.php';
    ...
    if(isset($_POST['email']))
    {
        ...
        if(preg_match($check, $spec) == FALSE)
        {
            $allIsOk = FALSE;
            $_SESSION['e_spec'] = "Specjalizacja może się składać tylko z liter i cyfr (Z polskimi znakami), musi zawierać minimum 2 znaki, oraz pierwsza litera musi być duża.";
        }
        if(strlen($spec)==0)
        {
            $allIsOk = FALSE;
            $_SESSION['e_spec'] = "To pole nie może pozostać puste.";
        }
        ...
        try
        {
             ...
             if ($connection->connect_errno!=0) 
                 ...
                 else
                 {
                    ...
                    $spec = mysqli_real_escape_string($connection, $spec);
                ...
                if( $allIsOk == TRUE)
                {
                    $specResult = $connection->query("SELECT `Id_lekarza`FROM `lekarze` WHERE Specjalnosc='$spec'");
                    if(!$specResult)  throw new Exception( $connection->error);
                     
                     $numberOfspec = $specResult->num_rows;
                     ...
                     {
                        $connection->query("INSERT INTO `lekarze`(`Specjalnosc`) VALUES ('$spec')");
                        $specResult = $connection->query("SELECT `Id_lekarza`FROM `lekarze` WHERE Specjalnosc='$spec'");
                        $specRow = mysqli_fetch_assoc($specResult);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////                
                    ...
                    if($connection->query("INSERT INTO `uzytkownicy`(`Id_uzytkownika`, `Imie`, `Nazwisko`, `E_mail`, `Login`, `Haslo`,    `Password_hash`,`Salt`, `Numer_telefonu`, `Data_urodzenia`, `Numer_domu`,     `Id_ulicy`,              `Id_pacjenta`, `Id_lekarza`,           `Id_obslugi`)"     
                                                         . " VALUES (     NULL,       '$name', '$surname', '$email', '$nick','$password1','$passwordHash','$salt','$phoneNumber',    '$dateOfBirth',  '$houseNumber' ,'$streetRow[Id_ulicy]',        NULL,      '$specRow[Id_lekarza]',     NULL)"))      
                    {
        ...
        <div id="container">
            ...
            <div id="login">
                ...
                <form method="post">
                    ...
                    Specjalizacja:</br> <input type="text" value="<?php
                    if(isset($_SESSION['fs_spec']))
                    {
                        echo $_SESSION['fs_spec'];
                        unset($_SESSION['fs_spec']);
                    }
                    ?>"  name="spec" /> </br>
                    <?php
                        if(isset($_SESSION['e_spec']))
                        {
                            echo '<div class="error"><span>'.$_SESSION['e_spec'].'</span></div>';
                            unset($_SESSION['e_spec']);
                        }
                    ?>
